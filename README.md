...
> 多源爬取 AI Agent 领域最新内容 → DeepSeek 翻译筛选摘要 → 推送到邮箱  
...
| # | 平台 | 做什么 | 拿到什么 |
|---|------|--------|---------|
| 1 | [DeepSeek](https://platform.deepseek.com/) | 注册 → API Keys | **API Key**（`sk-` 开头）|
| 2 | QQ 邮箱 | 设置 → 账户 → 开启 SMTP 服务 | **授权码**（一串字母，不是登录密码）|
...
> 💡 每次推送后会把已发链接记入 `history.json`，下次自动过滤掉重复内容。
...
| `SMTP_USER` | 你的 QQ 邮箱地址（如 `xxx@qq.com`）|
| `SMTP_PASSWORD` | QQ 邮箱「授权码」 |
| `SMTP_HOST` | `smtp.qq.com`（可选，默认即它）|
| `SMTP_PORT` | `465`（可选）|
| `MAIL_TO` | 收件邮箱（可选，默认发给自己）|
...
```
ai-news-pusher/
├── main.py              # 主程序（爬虫 + AI 处理 + 邮件推送）
├── requirements.txt     # 依赖（仅 requests）
├── .env.example         # 环境变量模板
├── history.json         # 去重历史（自动生成，勿手改）
